...
class BoxLooseDecoration extends PinDecoration implements SupportGap {
  ...
  void drawPin(
    ...
    var startY = mainHeight - strokeWidth / 2 + 9;

    /// Draw the each rect of pin.
    for (int i = 0; i < pinLength; i++) {
      if (i < text.length && enteredColor != null) {
        borderPaint.color = enteredColor;
        insidePaint.color = Colors.transparent;
      } else {
        insidePaint.color = Colors.white.withOpacity(0.1);

        borderPaint.color = Colors.white.withOpacity(0.1);
      }
